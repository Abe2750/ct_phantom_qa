...
def load_series(dicom_dir: str) -> DicomSeries:
    ...
    logger.info(
        f"Loaded {metadata['NumSlices']} slices, {metadata['Rows']}x{metadata['Columns']}, kVp={metadata['kVp']}"
    )

    return DicomSeries(middle_slice=middle_slice, metadata=metadata, slices=hu_slices)
